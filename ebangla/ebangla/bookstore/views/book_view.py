...
def edit_book(request,book_id):
    if request.method=='POST':
        name=Book.objects.filter(id=request.POST.get('id')).update(name=request.POST.get('name'))
        return HttpResponseRedirect('/')
    else:
        name=Book.objects.filter(id=book_id)
        data=RequestContext(request,{'first_name':request.session['first_name'],'todo':name[0]})
